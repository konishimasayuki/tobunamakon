...
export async function demoRequest(rawPath, options = {}) {
  ...
  if (path === '/api/debug') {
    ...
    if (method === 'POST' && q.id) { const t = db.debug[q.id]; if (!t) throw new Error('見つかりません'); const now = new Date().toISOString(); const imgs = Array.isArray(body.images) ? body.images : (body.image ? [body.image] : []); const nm = String(body.authorName || '').trim().slice(0, 40) || USER.username; t.replies.push({ id: uuid(), body: body.body || '', image: '', images: imgs, author: { id: USER.id, name: nm }, createdAt: now }); t.updatedAt = now; save(db); return t }
